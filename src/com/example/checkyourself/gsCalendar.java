...
public class gsCalendar extends Activity
{
	...
	TextView [ ] m_cellTextCountBtn;//각 칸마다 날짜가 표시되고 그 밑에 해당일에 몇개의 계획이 있는지 숫자를 보여주는 거.
...
////////////////////////////////////////    
	...
	static public class gsCalendarColorParam
	{
		...
		int m_topCellColor = 0xffff0000;// color of day
	...
	public gsCalendar( Context context, LinearLayout layout )
	{
		...
		m_cellTextCountBtn=new TextView[COLS*ROWS];
		m_horizontalLine = new LinearLayout[ COLS - 1 ] ;    /// 세로 구분하기위한 레이아웃
		m_verticalLine = new LinearLayout[ ( COLS - 1 ) * ROWS ] ; /// 가로를 구분하기위한  레이아웃
 
		// 화면의 크기에 따른 보정값
		m_displayScale = context.getResources( ).getDisplayMetrics( ).density ;
		System.out.println("size of m_displayScale:"+m_displayScale);
		...
		//월별 1일을 어떻게 찍는지 알고 나서 달력을 켰을때 해당 날짜가 하이라이트 되도록 표시해야함.
...
/// 선택된 날짜칸에 변화를 주기위한 함수 1호
	...
	public void setTodayOnCalendar(int cellColor,int textColor)
	{
	...
	public void createViewItem( )
	{
		...
		//셀에 들어갈 내용들을 만들어 놓고 경계선을 설정하도록 설계됨.
			...
			if( i % 2 == 0 )
			{
				...
				for( int j = 0 ; j < COLS * 2 - 1 ; j++ )
				{
					...
					if( j % 2 == 0 )
					{
						...
						m_cellTextCountBtn[pos]=new TextView(m_context);
						
						//셀에 날짜를 쓰는 TextView인 m_cellTxtBtn과 해당일의 일정 개수를 나타내는 m_cellTextCountBtn 넣
						m_cellLy[ pos ].addView(m_cellTextBtn[pos] );
						m_cellLy[pos].addView(m_cellTextCountBtn[pos]);
...
m_targetLayout.setBackgroundDrawable( m_bgImgId ) ;
	...
	for( int i = 0 ; i < ROWS * 2 - 1 ; i++ )
	{
		if( i % 2 == 0 )
		{
			...
			m_lineLy[i/2].setLayoutParams( new LinearLayout.LayoutParams( LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT ) ) ;
			...
			for( int j = 0 ; j < COLS ; j++ )
			{
				...
				/// 셀에 들어가는 텍스트의 배치방식 설정 
				m_cellTextBtn[cellnum].setGravity( Gravity.LEFT);
				
				m_cellTextCountBtn[cellnum].setGravity(Gravity.RIGHT);
				...
				else  //날짜 표시하는 부분
				{
					...
					m_cellTextCountBtn[cellnum].setLayoutParams( new LinearLayout.LayoutParams( m_cWidth*(1/3), m_cHeight*(1/3) ) );
					...
					//색상지정.
					m_cellTextCountBtn[cellnum].setTextColor(0xffffd700);
					
					//	/ 글씨 크기
					m_cellTextBtn[ cellnum ].setTextSize( m_textSize ) ;
					m_cellTextCountBtn[cellnum].setTextSize(m_textSize*(2/3));
				}
 
 
			}//for( int j = 0 ; j < COLS ; j++ )라
		}
	}
}
...
m_cellTextBtn[ k ].setText(  m_dayText[ k % 7 ] ) ;
	...
	/// 시작위치는 1부터 해서 달의 마지막날까지 숫자로 채움
	for( int i = 0 ; i < m_lastDay ; i++ )
	{
		m_cellTextBtn[ i + m_startPos ].setText( ( i + 1 ) + "" ) ;
		//여기에다가 일정 개수 count해서달력에 표시하도록 함.
		//그전에 데이터베이스에서 일정 개수 파악한 상태에 m_cellTextCountBtn에 값을 넣는다.
